...
describe("The Token contract", function () {
  ...
  it("Deployment should assign the total supply of tokens to the owner", async function () {
    ...
    expect(await hardhatToken.totalSupplies()).to.equal(ownerBalance); // Check if the total supply is equal to the balance of the owner, it means that we can check if the total supply is equal to the balance of the owner
  });

  it("Has the same name and Symbol", async function () {
    const [owner] = await ethers.getSigners(); // Signer is an account that can sign transactions, it means that it can deploy contracts
    ...
    expect(name).to.equal("The UNS Token")

    const symbol = await hardhatToken.symbol()
    expect(symbol).to.equal("TUT")
  });
  ...
  //   const [owner, addr1, addr2, addr3] = await ethers.getSigners(); // Get the owner account and two other accounts, it means that we can get the owner account and two other accounts

  //   const TheToken = await ethers.deployContract("TokenUNS"); // Deploy the contract, it means it will be deployed on the blockchain, and "TheToken" mean is the name of the contract
  //   expect(await TheToken.balanceOf(owner.address)).to.equal(await TheToken.totalSupplies()); // Check if the total supply is equal to the balance of the owner, it means that we can check if the total supply is equal to the balance of the owner
  ...
  //   // expect(await TheToken.balanceOf(addr2.address)).to.equal(50); // Check if the balance of addr2 is equal to 50, it means that we can check if the balance of addr2 is equal to 50

  //   // await TheToken.connect(owner).transfer(addr3.address, 500); // Transfer 50 tokens from the addr1 to addr2 account
  //   // expect(await TheToken.balanceOf(addr3.address)).to.equal(500); // Check if the balance of addr2 is equal to 50, it means that we can check if the balance of addr2 is equal to 50
  // });
